...
### A4 — the icon was drawn, looked at, and redrawn

**What changed.** Pass 1 specified a ring of radius ≈ 0.37 of the frame at
stroke ≈ 0.061, centred at (58%, 46%) "so the ring crops against the right
edge". Rendered, it did not: 0.58 + 0.37 = 0.95, which leaves a margin. At 60px
it read as a thin hoop adrift in a lot of empty buff — not findable, and not the
thing the sketch described.

**Now** radius 0.44 at stroke 0.095, centred at (60%, 50%), which genuinely
runs off the right edge, and the ink gaps are narrower so the ring reads as a
starved impression rather than a dashed circle. The renders at 180 / 120 / 60px
are checked into `docs/icon/` as evidence the inspection actually happened.

This is the sort of thing that only shows up when the artefact exists. The Pass
1 numbers were a description of an intention; these are the ones that produce it.

### A5 — the sounds are generated by a script in this repository

Both are synthesised by `scripts/make-sounds.mjs`: the shutter is two mechanical
transients over a filtered noise burst; the stamp is a low body that drops in
pitch as it lands, with a wooden mid resonance and a paper transient. They are
deterministic, so re-running the script reproduces them byte for byte.

The reason is licensing, not craft. A sound taken from a library arrives with a
provenance question attached; one that is the output of code in the repository
does not.

**The silent switch.** `expo-audio` defaults `playsInSilentMode` to **true**,
which overrides the physical switch. Respecting the switch therefore required
setting it to `false` deliberately — the default was the bug. There is a test
asserting it, because it is invisible until someone with a silenced phone is
startled by their own screen.
